...
function v2Auth(ak, sk, opt, log, methodName) {
  ...
  if (opt["urlPath"] !== "") {
    ...
    for (let i in arrPath) {
      ...
      if (
        allowedResourceParameterNames.indexOf(listvar[0].toLowerCase()) >= 0
      ) {
        let sep = urlPath === "" ? "?" : "&";
        urlPath += sep;
        if (listvar.length === 2) {
...
function getRequest(
  ...
  if (serverback.statusCode === 307 && !readable) {
    ...
    if (filePath) {
      ...
      contrustCommonMsg(opt, obj, headers);
      let respMsg =
        "Status: " + opt["CommonMsg"]["Status"] + ", headers: " + headersStr;
      log.interfaceLog(
        "debug",
        methodName,
        server,
                ...
                for (let key in obj) {
                  if (obj[key]["location"] === "xml") {
                    ...
                    if (
                      ...
                      opt["InterfaceResult"][key] = jsonToObject(
                        obj,
                        tempResult,
                        key,
                      )[key];
    ...
    if (key in param && param[key] !== null && param[key] !== undefined) {
      ...
      if (meta["location"] === "uri") {
        ...
        if (meta["type"] === "object") {
          ...
          exheaders[sentAs] = Buffer.from(param[key], "utf8").toString(
            "base64",
          );
...
Utils.prototype.sendRequest = function (funcName, opt, backcall, count) {
  ...
  this.makeRequest(funcName, opt, function (err, msg) {
    if (
      ...
      opt["port"] =
        uri.port ||
        (opt["protocol"].toLowerCase().indexOf("https") === 0 ? 443 : 80);
