...
ObsClient.prototype.uploadFile = function (param, callback) {
  ...
  var _callback = function (code, message, result) {
    ...
    if (typeof code === "string") {
      ...
      callback(
        code ? code.CommonMsg.Code + ":" + code.CommonMsg.Message : null,
        result,
      );
    }
  };
  ...
  eventEmitter.on("start to uploadFile", function (uploadCheckpoint) {
              ...
              function (err, result) {
                if (err || result.CommonMsg.Status >= 500) {
                  if (checkpointFile) {
                    _callback(
                      "IncompletedUpload",
                      err ? err : transResultToError(result),
                    );
                  } else {
                    abortRequest(
                      uploadCheckpoint,
            ...
            if (
              ...
              let currPartSize =
                start + 1 === uploadCheckpoint.partCount
                  ? uploadCheckpoint.uploadFileStat.fileSize - offset
              ...
              obsClient.uploadPart(
                ...
                function (err, result) {
                  ...
                  if (
                    checkpointFile &&
                    !writeCheckpointFileSync(checkpointFile, uploadCheckpoint)
                  ) {
                    return;
                  }
                  finishedCount++;
    ...
    if (!uploadId) {
      let contentType = uploadCheckpoint.contentType
        ...
        : obsClient.util.mimeTypes[key.substring(key.lastIndexOf(".") + 1)];
        ...
        fs.open(downloadTempFile, "a", function (err, fd) {
          ...
          let completedDownload = function (result) {
            if (finishedCount === downloadCheckpoint.partCount) {
              ...
              if (!hasError) {
                ...
                if (checkpointFile) {
                  if (
                    !writeCheckpointFileSync(checkpointFile, downloadCheckpoint)
                  ) {
                    return;
                  }
                  _callback(
            ...
            function (currentGroupIndex, groupCount, taskNum) {
              ...
              while (start < end) {
                ...
                if (
                  ...
                  downloadCheckpoint.partsMeta[i].isCompleted
                ) {
                  finishedCount++;
                  finishedCountGroup++;
                  let _taskNum =
                  ...
                  let endPos =
                    i === downloadCheckpoint.partCount
                      ? downloadCheckpoint.objectStatus.objectSize - 1
                      : i * downloadCheckpoint.partSize - 1;
                  obsClient.getObject(
                    {
          ...
          let groupCount =
            downloadCheckpoint.partCount % taskNum === 0
              ...
              : Math.floor(downloadCheckpoint.partCount / taskNum) + 1;
        ...
        if (!fs.existsSync(checkpointFile)) {
          ...
          fs.readFile(checkpointFile, function (err, data) {
            ...
            if (
              ...
              !downloadCheckpoint.objectStatus ||
              downloadCheckpoint.objectStatus.objectSize !== objectSize ||
              ...
              !verifyTempFileStatus(downloadCheckpoint.tempFileStatus) ||
              downloadCheckpoint.md5 !==
                calculateDownloadCheckpointMD5(downloadCheckpoint)
            ) {
              fs.unlink(checkpointFile, function () {});
